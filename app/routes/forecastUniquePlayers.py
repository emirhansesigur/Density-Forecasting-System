...
@router.post("/forecastUniquePlayers")
def ForecastUniquePlayersByDay(request: ForecastRequest):
    try:
        ...
        for hour in range(10, 22):
            ...
            predicted_unique_players = forecastingLogic(inputData, "players")

            hourlyPredictions.append({
                "hour": f"{hour:02d}:00",
                "predictedUniquePlayers": round(predicted_unique_players, 2),
                "temperature": weather["temperature"],
                "humidity": weather["humidity"],
                "precipitation": weather["precipitation"]
            })

        totalPredictedUniquePlayers = sum(item["predictedUniquePlayers"] for item in hourlyPredictions)

        return {
            "date": date.strftime("%Y-%m-%d"),
            "branchId": branchId,
            "branchName": branch_name,
            "totalPredictedUniquePlayers": round(totalPredictedUniquePlayers, 2),
